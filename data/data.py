...
def load_data(config: dict):
    ...
    # Construct absolute path if project_root_dir is in config (passed from main.py)
    ...
    target_column = config["data"].get("target_column", df.columns[1])
    if target_column not in df.columns:
        print(f"Warning: Target column '{target_column}' not found. Using first column '{df.columns[0]}'.")
    
    
    # Extract raw values (before scaling)
    raw_values = df[target_column].values.reshape(-1, 1) # Keep as 2D for scaler
