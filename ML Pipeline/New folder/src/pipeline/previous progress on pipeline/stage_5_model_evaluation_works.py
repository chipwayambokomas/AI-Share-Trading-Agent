...
def _evaluate_trend_prediction(model, X_test_t, y_test_t, stock_ids_test, scalers, config):
    ...
    predicted_scaled = model(X_test_t).numpy()
...
def _evaluate_point_prediction(model, X_test_t, y_test_t, stock_ids_test, scalers, config):
    ...
    predicted_scaled = model(X_test_t)
    actual_prices, predicted_prices = [], []
    for i in range(len(X_test_t)):
        stock_id = stock_ids_test[i]
        scaler = scalers[stock_id]
        actual_inv = scaler.inverse_transform(y_test_t.numpy()[i].reshape(-1, 1))
        predicted_inv = scaler.inverse_transform(predicted_scaled.numpy()[i].reshape(-1, 1))
        actual_prices.append(actual_inv.flatten()[0])
        predicted_prices.append(predicted_inv.flatten()[0])
