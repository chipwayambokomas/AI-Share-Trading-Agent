...
def _evaluate_trend_prediction(model, X_test_t, y_test_t, stock_ids_test, scalers, config):
    ...
    if config.MODEL_TYPE == 'GraphWaveNet':
        predicted_scaled = predicted_scaled.transpose(0, 2, 1, 3).squeeze(-1)
        ...
        actual_raw_slopes = scalers['slope'].inverse_transform(actual_slopes_scaled)
        actual_durations = scalers['duration'].inverse_transform(actual_durations_scaled)
        predicted_raw_slopes = scalers['slope'].inverse_transform(predicted_slopes_scaled)
        predicted_durations = scalers['duration'].inverse_transform(predicted_durations_scaled)

        actual_raw_slopes = actual_raw_slopes.flatten()
        actual_durations = actual_durations.flatten()
        predicted_raw_slopes = predicted_raw_slopes.flatten()
        predicted_durations = predicted_durations.flatten()
...
def _evaluate_point_prediction(model, X_test_t, y_test_t, stock_ids_test, scalers, config):
    ...
    if config.MODEL_TYPE == 'GraphWaveNet':
        ...
        for i in range(actual_scaled.shape[0]):
            for t in range(actual_scaled.shape[1]):
                for j in range(actual_scaled.shape[2]):
                    ...
                    dummy_actual[0, target_col_idx] = actual_scaled[i, t, j]
                    dummy_predicted[0, target_col_idx] = predicted_scaled[i, t, j]
...
def run(model, X_test_t, y_test_t, stock_ids_test, scalers, config, adj_matrix=None):
    ...
    if config.MODEL_TYPE == 'GraphWaveNet' and adj_matrix is not None:
