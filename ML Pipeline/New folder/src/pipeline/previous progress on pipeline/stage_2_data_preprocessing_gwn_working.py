...
1.  "POINT" Mode: Prepares multivariate data for price point prediction.
    ...
    - For GraphWaveNet, it creates graph-structured sequences.
2.  "TREND" Mode: Prepares data for trend line prediction using a highly
    ...
    - For GraphWaveNet, it creates a graph-structured representation of trends.
"""
import os
import time
import logging
...
def _run_trend_prediction_for_graph(df, config):
    ...
    # 1. Segment all stocks to get trends with start/end dates
    ...
    # 2. Create a daily "trend state" DataFrame
    ...
    # 3. Create adjacency matrix from trend slope correlations
    logger.info("Creating adjacency matrix from trend slope correlations...")
    correlation_matrix = daily_slopes.corr()
    adj_matrix = (correlation_matrix.abs() >= 0.5).astype(int) # Using a threshold of 0.5 for trends
    ...
    # 5. Combine into a single tensor (timesteps, nodes, features)
    graph_data = np.stack([scaled_slopes, scaled_durations], axis=-1)

    # 6. Create sequences for the graph model
    logger.info("Creating graph-structured sequences from trend data...")
    X, y = [], []
    in_win = config.TREND_INPUT_WINDOW_SIZE # Use trend window size
    out_win = 1 # We predict the next single trend state for all nodes
    total_len = in_win + out_win

    for i in range(len(graph_data) - total_len + 1):
        X.append(graph_data[i : i + in_win, :, :])
        y.append(graph_data[i + in_win, :, :]) # Target is the next state (t+1)
...
def _preprocess_for_graph_model(cleaned_df, config):
    ...
    The output shape for X is (num_samples, seq_len, num_nodes, num_features).
    """
...
def _run_point_prediction(df, config):
    ...
    if config.MODEL_TYPE == 'GraphWaveNet':
        ...
        adj_matrix = None # No adjacency matrix for non-graph models
