...
Executing Code: Loading and concatenating data from all sheets...
    ...
    for sheet_name in sheet_names:
        try:
            ...
            # 7. Get the date range before filtering.
            start_date = df['date'].min()
            end_date = df['date'].max()

            # 8. Filter out any stock that does not have a start date in 2014.
            if start_date.year != 2014:
                #print(f"    - SKIPPING: Stock '{sheet_name}' is skipped because its start date ({start_date.strftime('%Y-%m-%d')}) is not in 2014.")
                continue
            
            # 9. For valid stocks, print their date range.
            #print(f"    - PROCESSED: Stock '{sheet_name}' | Start Date: {start_date.strftime('%Y-%m-%d')} | End Date: {end_date.strftime('%Y-%m-%d')}")
            # --- END OF MODIFICATIONS ---

            # 10. Rename the 'date' column to 'Date' (capital D) to match downstream expectations.
            df.rename(columns={'date': 'Date'}, inplace=True)

            # 11. Select and reorder the final columns.
    ...
    if not processed_dfs:
        raise ValueError("No valid data could be loaded from any sheet in the Excel file. Check file format, column names, and start dates.")
