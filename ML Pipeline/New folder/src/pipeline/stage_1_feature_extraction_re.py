...
Executing Code: Loading and concatenating data from all sheets...
    ...
    for sheet_name in sheet_names:
        try:
            ...
            # 7. Define the target date range for the analysis.
            start_filter_date = pd.to_datetime('2019-01-01')
            end_filter_date = pd.to_datetime('2021-12-31')

            # 8. Filter the DataFrame to include only data within the specified date range.
            df = df[(df['date'] >= start_filter_date) & (df['date'] <= end_filter_date)]

            # 9. After filtering, check if the DataFrame for the stock is empty.
            if df.empty:
                #print(f"    - SKIPPING: Stock '{sheet_name}' has no data in the 2019-2021 range.")
                continue
