...
def _evaluate_trend_prediction(model, X_test_t, y_test_t, stock_ids_test, dates_test, scalers, config):
    ...
    with torch.no_grad():
        for X_batch_test, y_batch_test in test_loader:
            ...
            # --- START OF FIX: Add reshaping logic for HSDGNN ---
            if config.MODEL_TYPE == 'HSDGNN':
                if config.PREDICTION_MODE == 'POINT':
                    # This logic is for point prediction if you ever use it
                    ...
                    # Reshape from (B, H*D, N, 1) to (B, N, D)
    ...
    # This logic is now correct because predicted_scaled is guaranteed to be 3D
...
def run(model, X_test_t, y_test_t, stock_ids_test, dates_test, scalers, config, adj_matrix=None):
    ...
    if config.MODEL_TYPE in ['GraphWaveNet', 'DSTAGNN', 'HSDGNN'] and adj_matrix is not None:
        stock_ids_ordered = sorted(list(scalers.keys())) if isinstance(scalers, dict) and 'slope' not in scalers else stock_ids_test
