...
# --- END OF CRITICAL FIX ---
from pipeline import stage_1_feature_extraction2, stage_2_data_preprocessing2, stage_3_data_partitioning1, stage_5_model_evaluation1
from src import config_vwap
from pipeline import (
    stage_4_model_development_gwn_works
)
...
def main():
    ...
    df = stage_1_feature_extraction2.run(config_vwap)
    ...
    train_loader, val_loader, X_test_t, y_test_t, stock_ids_test = stage_3_data_partitioning1.run(X, y, stock_ids, config_vwap)
    ...
    model, training_duration = stage_4_model_development_gwn_works.run(train_loader, val_loader, config_vwap)

    # --- STAGE 5: Model Evaluation ---
    # This unified stage evaluates the model's predictions using metrics
    # relevant to the selected mode.
    stage_5_model_evaluation1.run(model, X_test_t, y_test_t, stock_ids_test, scalers, config_vwap)
