...
def main():
    ...
    # --- MODIFICATION FOR GRAPHWAVENET ---
    # The adjacency matrix needs to be passed to stage 4 as a list of "supports"
    supports = None
    if config.MODEL_TYPE == 'GraphWaveNet':
        if adj_matrix is not None:
            supports = [adj_matrix] # Wrap the matrix in a list
        else:
            raise ValueError("GraphWaveNet requires an adjacency matrix, but none was generated.")
    # ------------------------------------
