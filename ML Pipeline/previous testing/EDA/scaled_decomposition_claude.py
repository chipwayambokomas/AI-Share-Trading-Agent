...
# --- Scaling and Plotting Functions ---
...
def plot_trend_decomposition(df, column, symbol, split_name, output_dir, is_scaled=False):
    ...
    """
    # --- FIX: Dynamic period calculation ---
    # Use a smaller period for shorter time series to ensure decomposition can run.
    n_obs = len(df)
    if n_obs > 252: # More than a year of trading days
        period = 21 # Monthly
    elif n_obs > 60: # More than a quarter
        period = 5   # Weekly
    else:
        period = 2   # Minimal period
...
def run_analysis_visualizations(filled_split_data, scaled_data, output_dir):
    ...
    for symbol, splits in filled_split_data.items():
        ...
        for split_name, df_before in splits.items():
            ...
            # Plot decomposition for the data BEFORE scaling
            ...
            # Plot decomposition for the data AFTER scaling
...
if __name__ == "__main__":
    ...
    # --- FIX: Consolidated output directory ---
    ...
    if raw_split_data:
        ...
        print("   ├── 1_forward_filled_data/         (Unscaled CSVs)")
        print("   ├── 2_scaled_data/                 (Scaled CSVs)")
