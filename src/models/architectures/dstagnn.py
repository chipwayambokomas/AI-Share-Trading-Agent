...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask, res_att):
        ...
        attn_mask: [batch_size*num_of_d, n_heads, seq_len, seq_len]
        res_att: [batch_size*num_of_d, n_heads, len_q, len_k]
        '''
        scores = torch.matmul(Q, K.transpose(-1, -2)) / np.sqrt(self.d_k)  # scores : [batch_size*num_of_d, n_heads, len_q, len_k]
        
        # Add residual attention if provided and shapes match
        if res_att is not None:
            if res_att.shape == scores.shape:
                scores = scores + res_att
            else:
                # If shapes don't match, skip residual attention for this step
                pass
                
        if attn_mask is not None:
            scores.masked_fill_(attn_mask, -1e9)  # Fills elements of self tensor with value where mask is True.
        attn = F.softmax(scores, dim=-1)
        context = torch.matmul(attn, V)  # [batch_size*num_of_d, n_heads, len_q, d_v]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask, res_att):
        ...
        input_V: [batch_size, num_of_d, len_v(=len_k), d_model]
        attn_mask: [batch_size, seq_len, seq_len]
        '''
        residual, batch_size = input_Q, input_Q.size(0)
        num_of_d = input_Q.size(1)
        seq_len = input_Q.size(2)
        
        # Reshape to handle the num_of_d dimension properly
        # (B, F, T, N) -> (B*F, T, N) for processing
        input_Q_reshaped = input_Q.reshape(batch_size * num_of_d, seq_len, -1)
        input_K_reshaped = input_K.reshape(batch_size * num_of_d, seq_len, -1)
        input_V_reshaped = input_V.reshape(batch_size * num_of_d, seq_len, -1)
        
        # Project to Q, K, V
        Q = self.W_Q(input_Q_reshaped).view(batch_size * num_of_d, seq_len, self.n_heads, self.d_k).transpose(1, 2)
        K = self.W_K(input_K_reshaped).view(batch_size * num_of_d, seq_len, self.n_heads, self.d_k).transpose(1, 2)
        V = self.W_V(input_V_reshaped).view(batch_size * num_of_d, seq_len, self.n_heads, self.d_v).transpose(1, 2)
        
        # Reshape res_att to match Q, K dimensions
        if res_att is not None and res_att.numel() > 1:
            # Ensure res_att matches the expected dimensions
            expected_shape = (batch_size * num_of_d, self.n_heads, seq_len, seq_len)
            if res_att.shape != expected_shape:
                # Try to reshape if possible, otherwise create zero tensor
                try:
                    if res_att.numel() == batch_size * num_of_d * self.n_heads * seq_len * seq_len:
                        res_att = res_att.reshape(expected_shape)
                    else:
                        res_att = torch.zeros(expected_shape).to(input_Q.device)
                except:
                    res_att = torch.zeros(expected_shape).to(input_Q.device)
        else:
            res_att = torch.zeros(batch_size * num_of_d, self.n_heads, seq_len, seq_len).to(input_Q.device)
        
        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(1).repeat(1, self.n_heads, 1, 1)
            attn_mask = attn_mask.repeat(num_of_d, 1, 1, 1)
        
        # Apply attention
        context, res_attn = ScaledDotProductAttention(self.d_k, self.num_of_d)(Q, K, V, attn_mask, res_att)
        
        # Reshape back to original batch structure
        context = context.transpose(1, 2).reshape(batch_size * num_of_d, seq_len, self.n_heads * self.d_v)
        output = self.fc(context)
        
        # Reshape back to (B, F, T, N)
        output = output.reshape(batch_size, num_of_d, seq_len, -1)
        
        # Reshape res_attn back to (B, F, H, T, T)
        res_attn = res_attn.reshape(batch_size, num_of_d, self.n_heads, seq_len, seq_len)
...
class Embedding(nn.Module):
    ...
    def forward(self, x, batch_size):
        if self.Etype == 'T':
            # pos = torch.arange(self.nb_seq, dtype=torch.long).cuda() #commented out
...
class DSTAGNN_block(nn.Module):
    ...
    def __init__(self, DEVICE, num_of_d, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
        ...
        # self.adj_pa = torch.FloatTensor(adj_pa).cuda() #commented out

        self.adj_pa = torch.FloatTensor(adj_pa).to(DEVICE) #added this

        # The pre_conv should convert from temporal attention output to d_model dimensions
        # TATout shape after permute: (B, T, N, F) where F=num_of_d=features=5
        # Input to conv2d: (B, T, N, F) = (batch, 60, 32, 5)
        # Conv2d treats this as: (batch, in_channels=60, height=32, width=5)
        # So kernel should be (kernel_h, kernel_w) where kernel_w <= 5
    ...
    def forward(self, x, res_att):
        '''
        :param x: (Batch_size, N, F_in, T)
        :param res_att: residual attention tensor or scalar
        ...
        # Handle res_att initialization for first block
        if isinstance(res_att, (int, float)) and res_att == 0:
            res_att = None
            
        TATout, re_At = self.TAt(TEmx, TEmx, TEmx, None, res_att)  # B,F,T,N; B,F,Ht,T,T
        
        # Debug tensor shapes for different modes
        # print(f"TATout shape: {TATout.shape}")
        permuted_input = TATout.permute(0, 2, 3, 1)  # (B,F,T,N) -> (B,T,N,F)
        # print(f"Permuted input shape: {permuted_input.shape}")
        # print(f"Conv2d expects: channels={num_of_timesteps}, kernel=({1}, {num_of_d})")
        
        x_TAt = self.pre_conv(permuted_input)[:, :, :, -1].permute(0, 2, 1)  # B,N,d_model
...
class DSTAGNN(nn.Module):
    def __init__(self, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, 
                 ...
                 n_heads, d_k, d_model, adj_mx, adj_pa, adj_TMD, device, **kwargs):
        super(DSTAGNN, self).__init__()
        
        # Compute Chebyshev polynomials (what make_model used to do)
        L_tilde = scaled_Laplacian(adj_mx)
        cheb_polynomials = [torch.from_numpy(i).type(torch.FloatTensor).to(device) 
                           for i in cheb_polynomial(L_tilde, K)]
        
        # Create the submodule with parameters in EXACT order
        self.model = DSTAGNN_submodule(
            DEVICE=device,
            num_of_d=in_channels,  # This maps to your input features
            nb_block=nb_block,
            ...
            d_model=d_model,
            d_k=d_k,
            d_v=d_model//n_heads,  # Common pattern: d_v = d_model/n_heads
            n_heads=n_heads
        )
        
        # Parameter initialization (from make_model)
    ...
    def forward(self, x):
        """
        Forward pass
        :param x: Input tensor from data loader (B, T_in, N_nodes, F_in)
        :return: Model output (B, N_nodes, T_out)
        """
        # Permute the input from (B, T, N, F) to the expected (B, N, F, T)
        x = x.permute(0, 2, 3, 1)
