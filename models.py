...
class TCN_Forecaster(nn.Module):
    ...
    def forward(self, x):
        ...
        # TCN expects (batch_size, num_features, window_size)
...
class MLP_Forecaster(nn.Module):
    def __init__(self, input_size, hidden_layers, output_size, dropout=0.2):
        """
        A simple MLP (Multi-Layer Perceptron) for forecasting.
        """
    ...
    def forward(self, x):
        ...
        Input x: (batch_size, window_size, num_features)
        """
        # Flatten the input from (batch, window, features) to (batch, window * features)
